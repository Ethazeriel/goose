...
import fs from 'fs';
const { discord, spotify, napster, lastfm, root_url }:GooseConfig = JSON.parse(fs.readFileSync(fileURLToPath(new URL('../../config/config.json', import.meta.url).toString()), 'utf-8'));
